...
# 69
gcn_model = GCN(g, in_feats=12, n_hidden=64, n_classes=2, n_layers=2, activation=torch.nn.functional.relu)
maddpg = MADDPG(sim, device, actor_lr, critic_lr, hidden_dim, state_dims,
                action_dims, critic_input_dim, gamma, tau, gcn_model)
...
for i_episode in range(num_episodes):
    state = sim.reset()
    # print("state0", type(state[0]))
    state_ = torch.tensor(np.array(state))
    # print(state.shape)
    gcn_output = maddpg.gnn(state_)
