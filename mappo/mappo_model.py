...
class MAPPO:
    ...
    def choose_action(self, obs_n, evaluate):
        with torch.no_grad():
            actor_inputs = []
            obs_n = torch.tensor(obs_n, dtype=torch.float32)  # 观测observation输入 ：obs_n.shape=(N，obs_dim)
            ...
            prob = self.actor(actor_inputs)  # 输出选择动作概率 prob.shape=(N,action_dim)
